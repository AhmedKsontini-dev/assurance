...
const Sinistres = () => {
  ...
  const fetchSinistres = async () => {
    try {
      ...
      if (!response.ok) throw new Error('Failed to fetch sinistres');
      const data = await response.json();
      setSinistres(data);
    } catch (err) {
      showAlert('Erreur lors du chargement des sinistres', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async () => {
    try {
      const response = await fetch(`${API_URL}/sinistres/${sinistreToDelete}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
